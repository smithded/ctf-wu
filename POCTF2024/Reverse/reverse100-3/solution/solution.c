...
void print_flag_text(unsigned char *flag, int length) {
    for (int i = 0; i < length; i++) {
        printf("%c", flag[i]);
    }
    printf("\n");
...
int main() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        print_flag_hex(encoded_flag, length, i + 1);
        /*seed = seed / 10;
        if (seed == 0) {
            seed = 88974713;  // Reset seed if it runs out
        }*/
    ...
    printf("Decoded flag (plaintext): ");
    print_flag_text(encoded_flag, length);  // Print final decoded flag in text
